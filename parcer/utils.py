...
def create_table(connection, column_names, table_name):
    try:
        ...
        """.format(table_name, ", ".join("{} DECIMAL".format(column_name) for column_name in column_names)))
        connection.commit()
    ...
    try:
        for row in data:
            if not np.isnan(row).any():
                cursor.execute("INSERT INTO {} ({}) VALUES ({})".format(table_name, ','.join(column_names), ','.join(['%s'] * len(row))), row)
        connection.commit()
    except psycopg2.Error as e:
